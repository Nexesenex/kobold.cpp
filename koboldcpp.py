...
def autoset_gpu_layers(filepath,ctxsize,gpu0mem,blasbatchsize,flashattention,quantkv,mmqmode,lowvram,displaygpu,gpu1vram,gpu2vram,gpu3vram,poslayeroffset,neglayeroffset): #shitty algo to determine how many layers to use
    try:
        ...
        if fsize>10000000: #dont bother with models < 10mb
            ...
            print(f"Model size {fsize} B ; Context size: {cs} tokens")
            ...
            print(f"Context size: {cs} tokens ; Context compute buffer multiplier (CCBM): {csmul}")
            print("***")
            print(f"Offsets :")
            ...
            size_init = int(fsize*1.42*csmul)
            print(f"Model size {fsize} B x 1.42 x {csmul} CCBM = {size_init} B initially to load.")
            ratio_init = mem/size_init
            print(f"Initial ratio: {ratio_init} = GPU usable VRAM {mem} B / {size_init} B initially to load.")
            if ratio_init < 1:
                print(f"GPU usable VRAM : {mem} B < {size_init} B initially to load for the model + the context & compute buffers.")
                ...
                if not ggufmeta or ggufmeta[0]==0: #fail to read or no layers
                    ...
                    print(f"Size per layer: {sizeperlayer} B ; layers limit: {layerlimit} + offset of {layer_offset} layers.")
                    print("***")
                else:
                    ...
                    print("***")
                    if headcount > 0:
                        print("Precise calculations of the ratio:")
                    ...
                    print(f"Layers limit: {layerlimit} = final ratio {ratio} x {layers} layers + offset of {layer_offset} layers.")
                    estimated_loaded_size = int(layerlimit*sizeperlayer + total_buffer)
                    print(f"Estimated loaded size in the GPU: {estimated_loaded_size} B")
                    estimated_occupation_size = int(estimated_loaded_size + reserved_mem0)
                    print(f"Estimated size of the occupied memory of the GPU: {estimated_occupation_size} B")
            else:
                print(f"GPU usable VRAM : {mem} B > {size_init} B initially to load.")
...
#    makelabelentry(quick_tab, "V Cache config", cache_type_v_var, 1, 200, "Override the default f16 Value cache with quantized cache like q8_0, q5_1, q5_0, q4_1, or q4_0.")
    ...
    # context size
    makeslider(quick_tab, "Context Size:", contextsize_text, context_var, 0, len(contextsize_text)-1, 30, width=644, set=15,tooltip="What is the maximum context size to support. Model specific. You cannot exceed it.\nLarger contexts require more memory, and not all models support it.")
...
#    makecheckbox(tokens_tab, "Use Token Healing", token_healing, 3, tooltiptxt="Enable flash attention for GGUF models.")
    ...
    # context size
    makeslider(tokens_tab, "Context Size:",contextsize_text, context_var, 0, len(contextsize_text)-1, 20, width=644, set=15,tooltip="What is the maximum context size to support. Model specific. You cannot exceed it.\nLarger contexts require more memory, and not all models support it.")
    ...
    makefileentry(tokens_tab, "ChatCompletions Adapter:", "Select ChatCompletions Adapter File", chatcompletionsadapter_var, 32, width=512, filetypes=[("JSON Adapter", "*.json")], tooltiptxt="Select an optional ChatCompletions Adapter JSON file to force custom instruct tags.")
    ...
    makefileentry(model_tab, "Model:", "Select GGML Model File", model_var, 1,width=576, onchoosefile=on_picked_model_file,tooltiptxt="Select a GGUF or GGML model file on disk to be loaded.")
