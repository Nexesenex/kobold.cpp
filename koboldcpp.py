            ...
            try:
                with open(args.benchmark, "a") as file:
                    ...
                    if file.tell() == 0: #empty file
                        file.write(f"Datime,KCPP,LCPP,Backend,Model,NoAVX2,Thrd,HighP,NoBlas,Cublas1,Argument2,Argument3,Layers,Tensor1,Split2,BlasThrd,BBSize,FlashA,KVC,MaxCtx,GenNum,PPTime,PPSpeed,TGTime,TGSpeed,TotalTime,Coher,Output")
                    file.write(f"\n{ReleaseDate},{KcppVersion},{LcppVersion},{libname},{benchmodel},{args.noavx2},{args.threads},{args.highpriority},{args.noblas},{args.usecublas},,,{args.gpulayers},{args.tensor_split},,{args.blasthreads},{args.blasbatchsize},{args.flashattention},{args.quantkv},{benchmaxctx},{benchlen},{t_pp:.2f},{s_pp:.2f},{t_gen:.2f},{s_gen:.2f},{(t_pp+t_gen):.2f},{resultok},{result}")
            except Exception as e:
...
#    advparser.add_argument("--cache_type_v", help="Quantize the value cache, to lower the memory footprint of the context.", metavar=('[cache_type_v]'), nargs='+')  
    ...
    advparser.add_argument("--quantkv", help="Sets the KV cache data quantization type to save VRAM in NVidia Video Cards, 0=F16 (16 BPW), 8080=q8_0 (8.5 BPW), 8051=Kq8_0-Vq5_1 (7.25BPW), 8050=Kq8_0-Vq5_0 (7BPW), 8041=Kq8_0-Vq4_1 (6.75BPW), 8040=Kq8_0-Vq4_0 (6.5BPW), 5151=Kq5_1-Vq5_1 (6BPW), 5150=Kq5_1-Vq5_0 (5.75BPW), 5141=Kq5_1-Vq4_1 (5.5BPW), 5140=Kq5_1-Vq4_0 (5.25BPW), 5040=Kq5_0-Vq4_0 (5BPW), 4140=Kq4_1-Vq4_0 (4.75BPW), 4040=Q4_0 (4.5BPW). Requires Flash Attention, and disables context shifting.",metavar=('[quantization level F16 -> 0,8080,8051,8050,8041,8040,5151,5150,5141,5140,5040,4140,4040]'), type=int, choices=[0,8080,8051,8050,8041,8040,5151,5150,5141,5140,5040,4140,4040], default=0)
