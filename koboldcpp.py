            ...
            try:
                with open(args.benchmark, "a") as file:
                    ...
                    if file.tell() == 0: #empty file
                        file.write(f"Datime,KCPP,LCPP,Backend,Model,NoAVX2,Threads,HighP,Layers,TenSplit,BlasThreads,BBSize,FlashA,MaxCtx,GenNum,PPTime,PPSpeed,TGTime,TGSpeed,TotalTime,Coherence,Output")
                    file.write(f"\n{ReleaseDate},{KcppVersion},{LcppVersion},{libname},{benchmodel},{args.noavx2},{args.threads},{args.highpriority},{args.gpulayers},{args.tensor_split},{args.blasthreads},{args.blasbatchsize},{args.flashattention},{benchmaxctx},{benchlen},{t_pp:.2f},{s_pp:.2f},{t_gen:.2f},{s_gen:.2f},{(t_pp+t_gen):.2f},{resultok},{result}")
            except Exception as e:
...
if __name__ == '__main__':
    ...
    advparser.add_argument("--blasbatchsize", help="Sets the batch size used in BLAS processing (default 512). Setting it to -1 disables BLAS mode, but keeps other benefits like GPU offload.", type=int, default=512)
    ...
    advparser.add_argument("--smartcontext", help="Reserving a portion of context to try processing less frequently. But command is not garanteed to work anymore. instead, You can use --noshift instead to toggle smartcontext off on old GGML models.", action='store_true')
