...
static __global__ void soft_max_f32(
        const float * x, const T * mask, float * dst, const soft_max_params p, bool do_softcap) {
...
#pragma unroll
    for (int col0 = 0; col0 < ncols; col0 += block_size) {
        ...
        const float val = do_softcap ? p.scale*cap_params1*tanhf(cap_params0*x[ix]) + (mask ? slope*t2f32(mask[iy]) : 0.0f) :
...
template<typename T>
static void soft_max_f32_cuda(const float * x, const T * mask, float * dst, const soft_max_params & params, bool do_softcap, cudaStream_t stream) {
    int nth = WARP_SIZE;
    ...
    if (nbytes_shared < ggml_cuda_info().devices[ggml_cuda_get_device()].smpb) {
        ...
        soft_max_f32<false, 0, 0><<<block_nums, block_dims, nbytes_shared_low, stream>>>(x, mask, dst, params, do_softcap);
    }
}
...
void ggml_cuda_op_soft_max(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (use_f16) {
        ...
        soft_max_f32_cuda(src0_d, (const half  *) src1_d, dst_d, params, false, stream);
        ...
        soft_max_f32_cuda(src0_d, (const float *) src1_d, dst_d, params, false, stream);
    }
}
